...
public final class CacheStrategy {
    ...
    public static class Factory {
        ...
        private CacheStrategy getCandidate() {
            ...
            // 判断是否是Https请求并且缓存中的response是否丢失了必要的握手信息
            ...
            // 判断request和response是否能被缓存来生成CacheStrategy
            if (!isCacheable(cacheResponse, request)) {
                return new CacheStrategy(request, null);
            }

            CacheControl requestCaching = request.cacheControl();
            // 如果Request中CacheControl的noCache标志位为true，或者Request请求头中包含If-Modified-Since / If-None-Match
            // 所以可以在请求投中设置CacheControl 为 no-cache可以直接不使用缓存
            ...
            // 如果没有noCache标志位，并且缓存没有过期，该次不会进行网络请求，直接使用缓存
