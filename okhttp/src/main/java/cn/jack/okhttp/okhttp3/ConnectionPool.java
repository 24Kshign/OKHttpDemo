...
public final class ConnectionPool {
  ...
  // 双向队列，用来缓存所有的连接，Socket连接的创建和断开是有性能开销的，用一个队列来缓存连接，提高连接的利用率
  // 但是每次入队列的时候都会执行清理线程去清理一些无用的连接，因为最多只允许5个空闲的连接，而且只允许存活5分钟
  ...
  long cleanup(long now) {
    ...
    synchronized (this) {
      ...
      // 如果最长空闲的时间超过了设定的最大值，或者空闲链接数量超过了最大数量，则进行清理，否则计算下一次需要清理的等待时间
