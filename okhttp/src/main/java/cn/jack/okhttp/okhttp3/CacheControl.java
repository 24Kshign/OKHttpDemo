...
public final class CacheControl {
  ...
  private final boolean noCache;  // 对请求或响应消息不进行缓存
  private final boolean noStore;  // 数据不在硬盘中临时保存，对需要保密的内容比较重要
  private final int maxAgeSeconds;  // 告诉浏览器将页面缓存多长时间，超过这个时间后才再次向服务器发起请求检查页面是否有更新
  private final int sMaxAgeSeconds;  // 告诉缓存服务器（proxy，如Squid）的缓存页面的时间
  private final boolean isPrivate;  // 告诉proxy不要缓存，但是浏览器可以使用private cache进行缓存
  private final boolean isPublic;  // 告诉服务器，一些不该缓存的内容也要缓存，比如当用户已经认证时，所有的静态内容（图片、JavaScript、CSS等）应该是public的
  private final boolean mustRevalidate;
  private final int maxStaleSeconds;
  private final int minFreshSeconds;
  private final boolean onlyIfCached;  // 只使用缓存
  private final boolean noTransform;  // 告诉proxy不进行转换，比如告诉手机浏览器不要下载某些图片
  private final boolean immutable;  // 表示资源在有效期内服务器不会对其更改，这样客户端就不需要再发送验证请求头
