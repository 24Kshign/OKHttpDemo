...
import cn.jack.okhttp.okhttp3.ConnectionSpec;
import cn.jack.okhttp.okhttp3.EventListener;
import cn.jack.okhttp.okhttp3.Handshake;
...
import cn.jack.okhttp.okhttp3.internal.Version;
import cn.jack.okhttp.okhttp3.internal.http.HttpCodec;
import cn.jack.okhttp.okhttp3.internal.http.HttpHeaders;
import cn.jack.okhttp.okhttp3.internal.http1.Http1Codec;
import cn.jack.okhttp.okhttp3.internal.http2.ErrorCode;
import cn.jack.okhttp.okhttp3.internal.http2.Http2Codec;
import cn.jack.okhttp.okhttp3.internal.http2.Http2Connection;
import cn.jack.okhttp.okhttp3.internal.http2.Http2Stream;
import cn.jack.okhttp.okhttp3.internal.platform.Platform;
import cn.jack.okhttp.okhttp3.internal.tls.OkHostnameVerifier;
import cn.jack.okhttp.okhttp3.internal.ws.RealWebSocket;
...
public final class RealConnection extends Http2Connection.Listener implements Connection {
  ...
  // TCP TLS，区分Http/Https，Http就走Socket连接，Https需要额外进行TLS握手等操作
  public void connect(int connectTimeout, int readTimeout, int writeTimeout,
    ...
    if (route.address().sslSocketFactory() == null) {
      ...
      // 是否允许明文传输
      // 在Android 9.0以上不允许明文传输，于是乎就有了网上的解决方案
  ...
  private void connectTunnel(int connectTimeout, int readTimeout, int writeTimeout, Call call,
    ...
    // 创建隧道可能会不成功，所以这里设置了一个最大的创建次数21次
  ...
  private void establishProtocol(ConnectionSpecSelector connectionSpecSelector,
    ...
    // Http2请求需要额外进行TLS连接，进行数据加密传输与TLS握手等操作
  ...
  public boolean isEligible(Address address, @Nullable Route route) {
    ...
    // 当前的连接的最大并发数达到上限，或者被禁止创建新流了，则是不可用的连接
    if (allocations.size() >= allocationLimit || noNewStreams) return false;


    // 判断两个连接除host以外的变量是否相同，如果有不同的，那么连接也不能重用
