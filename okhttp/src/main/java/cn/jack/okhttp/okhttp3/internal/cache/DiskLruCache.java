...
import static cn.jack.okhttp.okhttp3.internal.platform.Platform.WARN;
...
public final class DiskLruCache implements Closeable, Flushable {
    ...
    private static final String CLEAN = "CLEAN";   // 稳定的缓存数据，可以读取
    private static final String DIRTY = "DIRTY";  // 不稳定的缓存数据，表示当前正在创建或者更新缓存文件
    private static final String REMOVE = "REMOVE";  // 移除缓存
    private static final String READ = "READ";  // 读取缓存
    ...
    private final File journalFile;  // 缓存日志文件，记录每次缓存的读，写操作
    ...
    synchronized Editor edit(String key, long expectedSequenceNumber) throws IOException {
        ...
        if (expectedSequenceNumber != ANY_SEQUENCE_NUMBER && (entry == null
            ...
            return null; // Snapshot is stale.  //快照不是新的
        }
    ...
    synchronized void completeEdit(Editor editor, boolean success) throws IOException {
        ...
        // 如果编辑器正在创建entry（Dirty还没写完）不允许提交
    ...
    boolean journalRebuildRequired() {
        ...
        // 当冗余日志超过日志文件本身的条数，一般总条数超过2000时就会触发清理线程
    ...
    private final class Entry {
         ...
         */
        final long[] lengths;
        //用来记录稳定状态的缓存，一般数组的长度为2，第一个是记录缓存的meta数据（像url，创建时间，握手信息等），第二个才用来记录真正的缓存数据
         ...
         */
        Editor currentEditor;  // 用来操作entry的
